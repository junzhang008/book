			...
			<head>
				<xsl:choose>
					<xsl:when test="kml:kml">
						<title>Locations Sitemap - Audiobook Factory</title>
					</xsl:when>
					<xsl:otherwise>
						<title>XML Sitemap - Audiobook Factory</title>
					</xsl:otherwise>
				</xsl:choose>
				...
				<xsl:choose>
					<xsl:when test="kml:kml">
						...
						<div id="content">
							...
							<p class="expl">
								<a href="http://audiobook-factory.local/sitemap_index.xml">&#8592; Sitemap Index</a>							</p>
